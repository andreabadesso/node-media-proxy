(function() {
    ...
    describe('Server', function() {
        ...
        describe('cryptedUrl', function() {
            ...
            it('The same URL should have the same crypt', () => {
                let hash1 = server._generateHash('rtsp://184.72.239.149/vod/mp4:BigBuckBunny_115k.mov');
                let hash2 = server._generateHash('rtsp://184.72.239.149/vod/mp4:BigBuckBunny_115k.mov');

                expect(hash1).to.equal(hash2);
            });

            it('Different URLs should not have the same crypt', () => {
                let hash1 = server._generateHash('rtsp://184.72.239.149/vod/mp4:BigBuckBunny_115k.mov');
                let hash2 = server._generateHash('rtsp://184.72.239.149/vod/mp4:BigBuckBunny_112k.mov');

                expect(hash1).to.not.equal(hash2);
            });

            it('Should find an empty port from redis', (done) => {
            ...
            it('Should find the last open port', (done) => {
            ...
            it('Should return an error when there are no empty ports', (done) => {
                // Limiting ports to a range out of the regular range
            ...
            it('Should find a port from the url hash', (done) => {
            ...
            it('Should not find a port from the url hash', (done) => {
            ...
            it('Port should be an integer', (done) => {
                redisClient.set('hash:melocotones', 8127, function(err) {
                    server._findHashStream('melocotones', function(err, port) {
                        expect(err).to.be.null;
            ...
            it('Should create an stream object on a random unused port', (done) => {
            ...
            it('Should get the stream port if it\'s already opened', (done) => {
                server._limitPorts(2010, 2010);

                let hash = server._generateHash('rtsp://184.72.239.149/vod/mp4:BigBuckBunny_115k.mov');
